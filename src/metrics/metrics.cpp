...
///
/// \param nodeId The unique identifier of the node for which the report path is
/// generated.
///
/// \return A std::filesystem::path representing the file path for the node
/// report.
...
void notifyReport(
  ...
  const double avgPowerConsumption =
      metrics.m_EnergyConsumption / metrics.m_ProcTime;
#define MFLOPS_TO_GFLOPS (1e-3)
  /// Calculates the machine's energy efficiency (GFLOPs/watts)
  const double energyEfficiency =
      (metrics.m_ProcMflops * MFLOPS_TO_GFLOPS) / avgPowerConsumption;
#undef MFLOPS_TO_GFLOPS
  ...
  report["type"] =
      ispd::services::getServiceTypeName(ispd::services::ServiceType::MACHINE);
...
void notifyReport(const ispd::metrics::LinkMetrics &metrics,
  ...
  report["type"] =
      ispd::services::getServiceTypeName(ispd::services::ServiceType::LINK);
  report["simulated_on"] = "node_" + std::to_string(g_tw_mynode);
...
void notifyReport(const ispd::metrics::MasterMetrics &metrics,
  ...
  const double avgTurnaroundTime =
      metrics.total_turnaround_time / metrics.completed_tasks;
  ...
  report["type"] =
      ispd::services::getServiceTypeName(ispd::services::ServiceType::MASTER);
  report["simulated_on"] = "node_" + std::to_string(g_tw_mynode);
